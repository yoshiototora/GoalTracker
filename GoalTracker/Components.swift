        ...
        VStack(spacing: 16) {
            HStack(alignment: .top) {
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    if let trend = comparisonText {
                        ...
                        Text(trend)
                            .font(.caption.bold())
                            .foregroundColor(isPositive ? .orange : .secondary)
                            .padding(.horizontal, 10)
                            .padding(.vertical, 4)
                            .background(isPositive ? Color.orange.opacity(0.1) : Color.gray.opacity(0.1))
                            .cornerRadius(20)
                            .lineLimit(1)
                ...
                ZStack {
                    ...
                    Image(systemName: total >= 0.8 ? "crown.fill" : (total >= 0.5 ? "star.fill" : "sparkles"))
                        .font(.title2)
                        .foregroundColor(total >= 0.8 ? .yellow : .orange.opacity(0.5))
            ...
            HStack(spacing: 12) {
                VStack(alignment: .leading) {
                    Text("完了タスク").font(.caption2).foregroundColor(.secondary)
                    Text("\(totalDoneCount)個").font(.headline).bold()
                }
                Divider().frame(height: 30)
                VStack(alignment: .leading) {
                    Text("Tryの実行").font(.caption2).foregroundColor(.secondary)
                    Text("\(tryDoneCount)回").font(.headline).bold().foregroundColor(.blue)
                }
                Spacer()
                
                Text(total >= 0.8 ? "最高のペース！" : (total >= 0.5 ? "いい調子です！" : "一歩ずつ進もう"))
                    .font(.caption2.bold())
...
struct CalendarGridView: View {
    ...
    func getCol(_ d: Date) -> Color {
        ...
        // 💡 修正：Goal型になったので、空かどうかは .title.isEmpty で判定します
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            if completedCount >= 1 {
                ...
                withAnimation(.easeOut(duration: 0.8).delay(0.3)) { showContent = true }
                withAnimation(.easeIn(duration: 1.0).delay(4.5)) { showContent = false }
            }
        }
