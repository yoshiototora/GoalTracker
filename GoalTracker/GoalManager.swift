...
class GoalManager: ObservableObject {
    ...
    func getTryExecutionCount(for date: Date, isWeekly: Bool) -> Int {
        ...
        return dates.reduce(0) { sum, d in
            sum + getNote(for: d).tasks.filter { $0.isCompleted && $0.type == .tryCarryOver }.count // 🔴 修正
        }
    }
    ...
    func syncGoalsToTasks(for date: Date) {
        ...
        note.tasks.removeAll { task in
            if task.type == .dailyGoal { return !currentDailyGoalTitles.contains(task.title) }
            if task.type == .tryCarryOver { return !allTryTitles.contains(task.title) }
            return false
        }
        
        for title in currentDailyGoalTitles {
            if !note.tasks.contains(where: { $0.title == title && $0.type == .dailyGoal }) {
                note.tasks.append(Task(title: title, type: .dailyGoal))
            }
        }
        for title in allTryTitles {
            if !note.tasks.contains(where: { $0.title == title && $0.type == .tryCarryOver }) {
                note.tasks.append(Task(title: title, type: .tryCarryOver))
            }
        }
        saveNote(note, for: date)
    }

    // 🔴 修正：大元のリストから消えたら、完了状態に関わらず削除
