                ...
                if currentTasks.isEmpty {
                    if !hasDailyGoals {
                        EmptyStateView(
                            ...
                            iconName: "target",
                            action: { selectedTab = 2; if tutorialStep == 0 { tutorialStep = 1 } }
                        )
                    } else {
                        EmptyStateView(
                            ...
                            ForEach(currentTasks) { task in
                                ...
                                HStack(spacing: 12) {
                                    ...
                                    if task.type == .dailyGoal {
                                        ...
                                        Text(task.title).strikethrough(task.isCompleted).foregroundColor(.blue)
                                    } else {
                                ...
                                .swipeActions(edge: .trailing, allowsFullSwipe: false) {
                                    if task.type == .normal {
                                        Button(role: .destructive) {
                                            if let idx = currentTasks.firstIndex(where: { $0.id == task.id }) { viewModel.removeTasks(at: IndexSet(integer: idx), for: viewModel.selectedDate) }
        ...
        NavigationView {
            VStack(spacing: 0) {
                ...
                TabView(selection: $reflectionType) {
                    dailyTab().tag(0)
                    if isSunday { weeklyTab().tag(1) }
                    if isLastDayOfMonth { monthlyTab().tag(2) }
    ...
    @ViewBuilder private func weeklyTab() -> some View {
        ...
        ScrollView {
            VStack(spacing: 15) {
                ...
                VStack(alignment: .leading, spacing: 10) {
                    GoalListSection(title: "先週のTryの実行チェック", iconColor: .gray, goals: viewModel.getLastWeeklyTryList(for: viewModel.selectedDate), viewModel: viewModel, showCheckboxes: true, templates: [], isReadOnly: true, onUpdate: { viewModel.updateWeeklyTryList($0, date: viewModel.getPreviousWeekDate(from: viewModel.selectedDate)) })
                    GoalListSection(title: "今週の目標チェック", iconColor: .orange, goals: weekData.goals, viewModel: viewModel, showCheckboxes: true, templates: weeklyTemplates, isReadOnly: true, onUpdate: { viewModel.updateWeeklyGoals($0, date: viewModel.selectedDate) })
                    ...
                    GoalListSection(title: "来週のTry", iconColor: .blue, goals: weekData.tryList, viewModel: viewModel, showCheckboxes: false, templates: [], onUpdate: { viewModel.updateWeeklyTryList($0, date: viewModel.selectedDate) })
    ...
    @ViewBuilder private func monthlyTab() -> some View {
        ...
        ScrollView {
            VStack(spacing: 15) {
                ...
                VStack(alignment: .leading, spacing: 10) {
                    GoalListSection(title: "先月のTryの実行チェック", iconColor: .gray, goals: viewModel.getLastMonthlyTryList(for: viewModel.selectedDate), viewModel: viewModel, showCheckboxes: true, templates: [], isReadOnly: true, onUpdate: { viewModel.updateMonthlyTryList($0, date: viewModel.getPreviousMonthDate(from: viewModel.selectedDate)) })
                    GoalListSection(title: "今月の目標チェック", iconColor: .blue, goals: monthData.monthlyGoals, viewModel: viewModel, showCheckboxes: true, templates: monthlyTemplates, isReadOnly: true, onUpdate: { viewModel.updateMonthlyGoals($0, field: .monthly, date: viewModel.selectedDate) })
                    TextEditorView(title: "今月のKeep", text: Binding(get: { monthData.keep }, set: { viewModel.updateMonthlyText($0, field: .keep, date: viewModel.selectedDate) })).focused($isKeyboardVisible)
                    TextEditorView(title: "今月のProblem", text: Binding(get: { monthData.problem }, set: { viewModel.updateMonthlyText($0, field: .problem, date: viewModel.selectedDate) })).focused($isKeyboardVisible)
                    GoalListSection(title: "来月のTry", iconColor: .blue, goals: monthData.tryList, viewModel: viewModel, showCheckboxes: false, templates: [], onUpdate: { viewModel.updateMonthlyTryList($0, date: viewModel.selectedDate) })
                    TextEditorView(title: "振り返り（自由記述）", text: Binding(get: { monthData.reflection }, set: { viewModel.updateMonthlyText($0, field: .reflection, date: viewModel.selectedDate) }), minHeight: 120).focused($isKeyboardVisible)
        ...
        NavigationView {
            TabView(selection: $monthOffset) {
                ForEach(-60..<61, id: \.self) { offset in
                    CalendarPage(viewModel: viewModel, offset: offset, displayDate: displayDate(for: offset), selectedTab: $selectedTab)
                        .tag(offset)
                }
            }
            .tabViewStyle(.page(indexDisplayMode: .never))
...
struct CalendarPage: View {
    ...
    var body: some View {
        ...
        ScrollView {
            VStack(spacing: 15) {
                HStack(spacing: 10) {
                    CompositeSummaryCard(title: "\(viewModel.getWeeklyTitle(for: viewModel.selectedDate))の達成度", rate1: viewModel.getWeeklyDailyAvgRate(for: viewModel.selectedDate), rate2: viewModel.getWeeklyGoalRate(for: viewModel.selectedDate), rate3: nil, color2: .orange, color3: .clear, comparisonText: viewModel.getComparisonText(for: viewModel.selectedDate, isWeekly: true))
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    ScrollView(.horizontal, showsIndicators: false) {
                        HStack(spacing: 10) {
                            BatonTag(title: "先月より", items: viewModel.getLastMonthlyTryList(for: displayDate).map { $0.title }, color: .blue)
                            BatonTag(title: "先週より", items: viewModel.getLastWeeklyTryList(for: viewModel.selectedDate).map { $0.title }, color: .orange)
                            BatonTag(title: "昨日より", items: viewModel.getYesterdayTryList(for: viewModel.selectedDate), color: .green)
                        }.padding(.horizontal)
                    }
                }.padding(.vertical, 5)
